...
(define (indegree Es v)
 (cond (( null? Es) 0)
       ((eq? (cadar Es) v) (+ 1 (indegree (cdr Es) v)))
...
(define (find-top G)
  (find-topHelper (get-vertices G) (get-edges G)))

(define (find-topHelper V Es)
  (cond ((null? V) (tsort-error))
        ((eq? (indegree Es (car V)) 0) (car V))
        (else (find-topHelper (popv-vertices V (car V)) Es))))
