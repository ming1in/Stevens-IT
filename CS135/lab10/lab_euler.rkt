...
(define (degree G v)
(cond ((null? G) 0)
      ((and (eq? v (caar G)) (eq? v (cadar G))) (+ 2 (degree (cdr G) v)))
      ((member v (car G)) (+ 1 (degree (cdr G) v)))
      (else (degree (cdr G) v)))
  )
...
(define (all-even-helper G V)
  (cond ((null? V) #t)
        ((even? (degree G (car V))) (all-even-helper G (cdr V)))
        (else #f))
  )
...
(define (adjacentHelper G v)
  (cond ((null? G) '())
        ((eq? (caar G) v) (append (list (cadar G)) (adjacentHelper (cdr G) v)))
        ((eq? (cadar G) v) (append (list (caar G)) (adjacentHelper (cdr G) v)))
...
(define (connected? G)
  (equal? (get-vertices G) (connectedHelper G)))
